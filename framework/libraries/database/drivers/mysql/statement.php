...
final class Statement extends \Library\Database\Results {
    ...
    public function listColumns() {
        $fieldNames = array();
        $field = NULL;
        while ($field = mysql_fetch_field($this->getResultId())) {
    ...
    public function freeResults() {
        if (is_resource($this->getResultId())) {
    ...
    public function dataSeek($n = 0) {
        return mysql_data_seek($this->getResultId(), $n);
    }
    ...
    public function fetchAssoc() {
        ...
        return mysql_fetch_array($this->getResultId(), MYSQL_ASSOC);
    }
