 ...
 * @category   Utility
 * @author     Livingstone Fultang <livingstone.fultang@stonyhillshq.com>
 * @author     Original Source from var/spool Wrench Library 
 * @authorurl  https://github.com/varspool/Wrench/blob/master/lib/Wrench/Protocol/Protocol.php
 * @copyright  1997-2012 Stonyhills HQ
 * @license    http://www.gnu.org/licenses/gpl.txt.  GNU GPL License 3.01
 * @version    Release: 1.0.0
...
abstract class Specification {
    ...
    $uri, $key, $origin, array $headers = array()
    ) {
        if (!$uri || !$key || !$origin) {
    ...
    $request
        ...
        if (!isset($headers[self::HEADER_UPGRADE]) || strtolower($headers[self::HEADER_UPGRADE]) != self::UPGRADE_VALUE
        ) {
        ...
        if (!isset($headers[self::HEADER_CONNECTION]) || stripos($headers[self::HEADER_CONNECTION], self::CONNECTION_VALUE) === false
        ) {
    ...
    protected function validateScheme($scheme) {
        if (!$scheme) {
            throw new Exception('No scheme specified');
        }
    ...
    protected function getPort($scheme) {
        if ($scheme == self::SCHEME_WEBSOCKET) {
            ...
            throw new Exception('Unknown websocket scheme');
        }
    }
