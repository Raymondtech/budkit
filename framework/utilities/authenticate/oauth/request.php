...
class Request {
     ...
     * @param  string  request URL
     * @param  array   request parameters
     * @uses   Authenticate\OAuth::parse_url
     */
    public function __construct($method, $url, array $params = NULL) {
        ...
        if ($this->required('oauth_version') AND !isset($this->params['oauth_version'])) {
            ...
            $this->params['oauth_version'] = Authenticate\OAuth::$version;
        }
     ...
     * @return  string
     * @uses    Authenticate\OAuth::urlencode
     * @uses    Authenticate\OAuth::normalize_params
     */
    ...
    public function asHeader() {
        ...
        foreach ($this->params as $name => $value) {
            if (strpos($name, 'oauth_') === 0) {
                ...
                $header[] = Authenticate\OAuth::urlencode($name) . '="' . Authenticate\OAuth::urlencode($value) . '"';
            }
        }
    ...
    public function check() {
        foreach ($this->required as $param => $required) {
            if ($required AND !isset($this->params[$param])) {
                throw new \Platform\Exception(sprintf('Request to %s requires missing parameter "%s"', $this->url, $param));
            }
        }
    ...
    public function execute(array $options = NULL) {
        ...
        return Authenticate\OAuth::remote($url, $options);
    }
}