...
!function($) {
    "use strict"
    var BKUploader = function(form, options) {
        this.options = $.extend({}, $.fn.bkuploader.defaults, options);
        this.form = $(form);
        this.selector = $('.upload-selected-files'); 
        
        //Click to upload
        $(this.form.find('.upload-select')).on("click", function(event){
            event.preventDefault();
            $('.upload-selected-files').click();
        });
        $(this.form.find('.upload-selected-files')).on('change', $.proxy(this.getSelectedFiles, this));
    ...
    BKUploader.prototype = {
        ...
        getSelectedFile: function(selector){
            console.log( selector.length );
        },
        getSelectedFiles : function(event){
            event.preventDefault();
            if (!(this.selector instanceof $) || this.selector.length === 1) {
                return this.getSelectedFile(this.selector);
            }
            return $.when.apply(
                $,
                $.map(this.selector, this.getSelectedFile) ).pipe(function () {
