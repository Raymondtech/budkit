...
!function($) {
    ...
    var BKEditor = function(element, options) {
        ...
        if (this.options.stylesheet) {
            var e = edit.createElement('link'); e.rel = 'stylesheet'; e.type = 'text/css'; e.href = this.options.stylesheet; edit.getElementsByTagName('head')[0].appendChild(e);
        }
        
        //this.copyAttributes(["spellcheck", "value", "placeholder"]).from( this.element ).to(this.editor.body);
        $(edit.body).attr("contenteditable", true).on('click keyup keydown mousedown blur', function(){ $editor.updateElement() })
    ...
    BKEditorToolbar.prototype =  {
        commands : {
            bold : ['bold', 'Bold', function($this){ $this.ec("bold") }],//class, title, onClick, onAfterExecute,
            italic : ['italic', 'italic', function($this){ $this.ec("italic")  }],
            underline : ['underline', 'Underline', function($this){ $this.ec("underline"); }],
            strikethrough : ['strikethrough', 'Strike through',function($this){ $this.ec("strikethrough"); }],
            orderedlist: ['list-ol', 'Insert ordered list',function($this){ $this.ec("insertorderedlist"); }],
            unorderedlist:['list',  'Insert unordered list',function($this){ $this.ec("insertunorderedlist"); }],
